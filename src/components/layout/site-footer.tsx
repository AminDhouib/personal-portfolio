...
function LinkedInIconFooter({ className }: { className?: string }) {
  return (
...
function ContraIconFooter({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
...
function FooterSocialIcon({ icon }: { icon: string }) {
  if (icon === "linkedin") return <LinkedInIconFooter className="h-4 w-4" />;
  if (icon === "contra") return <ContraIconFooter className="h-4 w-4" />;
  return <SiIcon slug={icon} className="h-4 w-4" />;
}

export function SiteFooter() {
  const pathname = usePathname();
  // Homepage Contact section already renders large social icons + copyright,
  // so suppress the global footer there to avoid a visual duplicate.
