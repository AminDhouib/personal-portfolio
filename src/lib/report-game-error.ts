 ...
 *
 * `nextGameCrash` centralizes two things a `catch` at the top of a game's
 ...
 * `const c = nextGameCrash(game, err); if (c) reportError(c);` so
 * `no-silent-catch` sees a literal reporter call in every wrapped catch.
 */

const reportedGames = new Set<string>();

export function nextGameCrash(game: string, error: unknown): Error | null {
